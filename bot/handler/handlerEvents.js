...
// ── Commands allowed to run even when the bot is stopped in a thread ──────────
// "stop" handles +stop and +resume (via aliases), so only "stop" is needed here
const STOP_EXEMPT_COMMANDS = ["stop", "resume"];
        ...
        return async function (event, message) {
                ...
                */
                ...
                async function onStart() {
                        ...
                        // ── CHECK IF BOT IS STOPPED IN THIS THREAD ────────────────
                        // Allow stop/resume through so admins can always undo it.
                        // Everyone else is silently blocked.
                        if (global.stoppedThreads?.[threadID]) {
                                const args_ = body.slice(prefix.length).trim().split(/ +/);
                                const cmdWord = args_[0]?.toLowerCase();
                                const resolvedCmd = GoatBot.commands.get(cmdWord) || GoatBot.commands.get(GoatBot.aliases.get(cmdWord));
                                const resolvedName = resolvedCmd?.config?.name;
                                if (!STOP_EXEMPT_COMMANDS.includes(resolvedName)) {
                                        return; // silently ignore all non-exempt commands
                                }
                        }
                        // ─────────────────────────────────────────────────────────
                ...
                */
                async function onChat() {
                        // ── BLOCK onChat handlers when bot is stopped in this thread ──
                        if (global.stoppedThreads?.[threadID])
                                return;
                        // ──────────────────────────────────────────────────────────────
